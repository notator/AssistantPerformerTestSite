...
//export function readSoundFontFileAsArrayBuffer(soundFontURL, soundFontName, presetIndices, resolveCallback, rejectCallback)
export function readSoundFontFileAsArrayBuffer(soundFontURL, soundFontName, presetIndices)
{
...
{
	constructor(soundFontName, arrayBuffer, presetIndices)
	{
		function createBagModGen_(indexStart, indexEnd, zoneModGen)
		{
			var modgenInfo = [],
				modgen = {
					unknown: [],
		...
		function getPresetModulator_(parser, zone, index)
		{
		...
		function getPresetGenerator_(parser, zone, index)
		{
		...
		function createInstrumentModulator_(parser, zone, index)
		{
			var modgen = parser.createBagModGen_(
				zone,
				zone[index].presetModulatorIndex,
		...
		function createInstrumentGenerator_(parser, zone, index)
		{
			var modgen = parser.createBagModGen_(
				zone,
		...
		function getBanks(uint8Array, nRequiredPresets)
			{
		...
		function getPresetInfo(presetIndices)
		{
			...
			for(i = 0; i < presetIndices.length; ++i)
			{
				...
				name = constants.generalMIDIPatchName(presetIndex);
				soundFontPresets.push({ name: name, presetIndex: presetIndex });
			}
			return soundFontPresets;
		}

		let presets = getPresetInfo(presetIndices),
			uint8Array = new Uint8Array(arrayBuffer),
			banks = getBanks(uint8Array, presets.length);

		Object.defineProperty(this, "name", { value: soundFontName, writable: false });
		Object.defineProperty(this, "banks", { value: banks, writable: false });
		Object.defineProperty(this, "presets", { value: presets, writable: false });
	}
}
